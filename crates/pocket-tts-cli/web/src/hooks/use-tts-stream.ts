...
export function useTTSEngine() {
	...
	const tuneWasmThresholds = useCallback(
		(ttfaMs: number | null, rebuffers: number) => {
			if (rebuffers > 0) {
	...
	const initAudio = useCallback(
		async (
			...
			if (
				audioCtxRef.current &&
				Math.round(audioCtxRef.current.sampleRate) !== desiredRate
			) {
	...
	const prepareVoice = useCallback(
		async (input: VoicePreparationInput): Promise<string | undefined> => {
	...
	const generate = useCallback(
		async (text: string, voiceSpec?: string) => {
			...
			const resumeThresholdSec =
				mode === "wasm-experimental" ? wasmResumeThresholdSecRef.current : 0.45;
			setPlaybackStats((prev) => ({
				...prev,
				rebufferCount: 0,
				startThresholdSec,
			...
			try {
				...
				if (mode === "wasm-experimental") {
					tuneWasmThresholds(
						latencyRef.current.ttfaMs,
						rebufferCountRef.current,
					);
				}
	...
	const runTtfaProbe = useCallback(
		async (text: string, voiceSpec?: string): Promise<LatencyMetrics> => {
