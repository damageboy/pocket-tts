...
export default function App() {
	...
	const [hfRepo, setHfRepo] = useState(() => {
		return (
			window.localStorage.getItem(HF_REPO_STORAGE_KEY) || "kyutai/pocket-tts"
		);
	});
	...
	const [manualTokenizerFile, setManualTokenizerFile] = useState<File | null>(
		null,
	);
	...
	const buildVoicePreparationInput = async () => {
		...
		const embeddingBytes = isWasmMode
			? await readFileBytes(embeddingFile)
			: null;
						...
						<div>
							...
							<p className="text-xs text-muted-foreground font-medium flex items-center gap-2">
								...
								{isWasmMode && (
									<Badge
										variant="outline"
										className="text-[10px] uppercase border-amber-500/40 text-amber-600"
									>
										Experimental WASM
									</Badge>
								)}
				...
				<div className="grid gap-6">
								...
								<div className="space-y-4 rounded-lg border border-amber-500/20 bg-amber-500/5 p-4">
									<div className="flex items-start justify-between gap-4">
										<div>
											...
											<p className="text-xs text-muted-foreground mt-1">
												Experimental browser-side inference. Initialize once
												before generating.
											</p>
										</div>
										<Button
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between text-[11px]">
											<span className={wasmStatusTone}>
												{wasmLoadStatus.message}
											</span>
											<span className="font-mono text-muted-foreground">
												{Math.round(wasmLoadStatus.progress)}%
											</span>
										</div>
										<Progress value={wasmLoadStatus.progress} className="h-2" />
										<div className="text-[10px] text-muted-foreground">
											Source: {wasmLoadStatus.source || "n/a"} | Phase:{" "}
											{wasmLoadStatus.phase}
										</div>
									</div>
									...
									<div className="grid md:grid-cols-2 gap-3">
										<div className="space-y-2">
											<Label htmlFor="hf-repo" className="text-xs">
												HF Repository
											</Label>
											<Input
												id="hf-repo"
											...
											<div className="p-3 grid md:grid-cols-3 gap-3 border-t border-muted-foreground/10">
												<div className="space-y-1">
													...
													<Input
														...
														accept=".yaml,.yml"
														onChange={(e) =>
															setManualConfigFile(e.target.files?.[0] || null)
														}
													/>
												</div>
												<div className="space-y-1">
													<Label
														htmlFor="manual-weights"
														className="text-[10px] text-muted-foreground"
													>
														Model Weights
													</Label>
													<Input
														...
														accept=".safetensors"
														onChange={(e) =>
															setManualWeightsFile(e.target.files?.[0] || null)
														}
													/>
												</div>
												<div className="space-y-1">
													<Label
														htmlFor="manual-tokenizer"
														className="text-[10px] text-muted-foreground"
													>
														Tokenizer JSON
													</Label>
					...
					<Card className="border-muted-foreground/10 bg-card/40 backdrop-blur-xl">
						<CardHeader className="pb-3">
							...
							<CardDescription>
								Measure Time-To-First-Audio for the 600ms target requested by
								maintainers.
							</CardDescription>
						</CardHeader>
							...
							<div className="grid grid-cols-3 gap-2 text-xs">
								<div className="rounded border border-muted-foreground/10 p-2">
									...
									<div className="font-mono mt-1">
										{probeResult?.ttfcMs != null
											? `${probeResult.ttfcMs.toFixed(0)}ms`
											: "-"}
									</div>
								</div>
								<div className="rounded border border-muted-foreground/10 p-2">
									...
									<div className="font-mono mt-1">
										{probeResult?.ttfaMs != null
											? `${probeResult.ttfaMs.toFixed(0)}ms`
											: "-"}
									</div>
								</div>
								<div className="rounded border border-muted-foreground/10 p-2">
									...
									<div className="font-mono mt-1">
										{probeResult?.totalMs != null
											? `${probeResult.totalMs.toFixed(0)}ms`
											: "-"}
									</div>
								</div>
							</div>
